...
The dataset at hand has `r nrow(car_data)` observations 
...
### A linear regression model estimated to run through the origin suggests the nonexistence of a constant, or intercept, following the assumption that such an intercept shall not conceptually apply to the data set being modeled. The linear regression model depicted in e) looks as follows (include model throug LaTeX here). The model does not include an intercept beta0. When looking at the conceptual context of the model, it is intuitive to see that an intercept should not be included. A car with no cylinder volume (liter) should have a selling price (price) of 0, as it either has no engine or is something other than a car.
